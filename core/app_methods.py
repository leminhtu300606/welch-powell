...
class AppMethods:
    ...
    def render_graph(self):
        ...
        # Lấy thông tin đường đi đang được chọn (nếu có)
        h_path = getattr(self, "highlighted_path", [])
        h_color = getattr(self, "highlighted_color", "red")
        
        # Tạo set chứa các cạnh được highlight để kiểm tra nhanh
        h_edges = set()
        for i in range(len(h_path) - 1):
            h_edges.add((min(h_path[i], h_path[i+1]), max(h_path[i], h_path[i+1])))
        ...
        for edge in self.edges:
            ...
            # Nếu cạnh này nằm trong đường đi, tô màu và làm dày lên
            edge_pair = (min(edge["node1_id"], edge["node2_id"]), max(edge["node1_id"], edge["node2_id"]))
            is_highlighted = edge_pair in h_edges
            
            e_color = h_color if is_highlighted else "gray"
            e_width = 4 if is_highlighted else 2
            
            edge["line"] = self.canvas.create_line(cx1, cy1, cx2, cy2, fill=e_color, width=e_width, tags="edge")
            
            if getattr(self, "algorithm_mode", "") == "dijkstra" and "weight" in edge:
                mx, my = (cx1 + cx2) / 2, (cy1 + cy2) / 2
                self.canvas.create_rectangle(mx-8, my-10, mx+8, my+10, fill="white", outline="white")
                self.canvas.create_text(mx, my, text=str(edge["weight"]), fill="#c0392b", font=("Arial", 10, "bold"))

        for node in self.nodes:
            # Nếu đỉnh nằm trong đường đi, tô viền màu tương ứng
            ...
            node["circle"] = self.canvas.create_oval(cx - radius, cy - radius, cx + radius, cy + radius, fill=node["color"], outline=outline_color, width=outline_width)
            node["text"] = self.canvas.create_text(cx, cy, text=node["label"], fill="black", font=("Arial", 12, "bold"))
